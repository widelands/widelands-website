...
class SuspiciousModelTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.test_user = User.objects.create_user("donald", "donald@duck.de",
                                                 "donaldpwd")
        cls.forum_category = Category.objects.create(name="forum_cat",
                                                     position=1, internal=False)
        cls.forum_forum = Forum.objects.create(category=cls.forum_category)
        cls.forum_topic = Topic.objects.create(forum=cls.forum_forum,
                                               user=cls.test_user,
                                               name="test spam")
        cls.forum_post = Post.objects.create(topic=cls.forum_topic,
                                             user=cls.test_user, body="testing")

    def test_spam_topic(self):
        spam_topic_text_with_spam = "This topic is spammin"
        susp_input = SuspiciousInput(content_object=self.forum_topic,
                                     user=self.test_user,
    ...
    def test_no_spam_post(self):
        ...
        susp_input = SuspiciousInput(content_object=self.forum_post,
                                     user=self.test_user,
    ...
    def test_suspicious_text_length(self):
        ...
        self.assertEqual(len(susp_input.text),
