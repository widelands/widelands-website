...
def make_graph(tribe_name):
    ...
    with open(
        path.normpath(json_directory + "/tribe_" + tribe_name + ".json"), "r"
    ) as tribeinfo_file:
...
def make_building_graph(t, building_name):
    ...
    if not isinstance(b, ProductionSite):
        ...
        # and output.
        inputs, outputs = [
            [t.wares[name] for name in lst] for lst in [b.inputs, b.outputs]
        ]
...
def make_worker_graph(t, worker_name):
    ...
    buildings = [
        bld
        for bld in list(t.buildings.values())
        if isinstance(bld, ProductionSite)
        and (w.name in bld.workers or w.name in bld.recruits)
    ]
    ...
    for bld in buildings:
        add_building(
            ...
            limit_inputs=[],
            limit_outputs=[],
            limit_buildings=[buildings],
            link_workers=False,
            limit_recruits=[w.name],
        )
...
def make_ware_graph(t, ware_name):
    ...
    buildings = [
        ...
        for bld in list(t.buildings.values())
        if isinstance(bld, ProductionSite)
        and (w.name in bld.inputs or w.name in bld.outputs)
    ]
    [
        add_building(
            g,
            bld,
            limit_inputs=[w.name],
            limit_outputs=[w.name],
            limit_buildings=[b.name for b in buildings],
            link_workers=False,
        )
        for bld in buildings
    ]
