...
"""
    ...
    def runTest(self):
        self.assertRaises(
            InvalidMapPackage, self.m._read_elemental, StringIO(self.input)
        )
...
class TestHeightReading_ValidInput_ExceptCorrectResult(_WLHeight_Base):
    ...
    dim = (4, 6)  # w6, h4
    input = (
        "\x01\x00)!\x08*\x0b1\x18-0\x1d\x03\x08.\x1e\x01\x19\x1f\x1d\x17\x17\x1c\x07#5"
    )
    wanted = array(
        [
            [41, 33, 8, 42, 11, 49],
...
class TestTerrainReading_ValidInput_ExceptCorrectResult(_WLTerrain_Base):
    ...
    input = (
        "\x01\x00"
        + "\x02\x00"  # Package version
        + "\x00\x00Baum\x00\x01\x00Haus\x00"  # Nr of terrain types
        + "\x01\x00\x00\x01"  # Terrains (id,Name\x00)  # Terrain data (ter_r,ter_d)*nr_of_fields
    )
    ...
    def test_R(self):
        r = [i.name == j for i, j in zip(self.m.ter_r.flat, self.ter_r_names.flat)]
        self.assertTrue(all(r))

    def test_D(self):
        d = [i.name == j for i, j in zip(self.m.ter_d.flat, self.ter_d_names.flat)]
...
class TestTerrainReading_WrongTerrainId(_WLTerrain_Setup):
    ...
    input = (
        ...
        + "\x02\x00"  # Package version
        + "\x00\x00Baum\x00\x01\x00Haus\x00"  # Nr of terrain types
        + "\x00\x00\x02\x00"  # Terrains (id,Name\x00)  # Terrain data (ter_r,ter_d)*nr_of_fields
    )
