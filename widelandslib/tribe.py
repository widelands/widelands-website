...
class Building(BaseDescr):
    ...
    def base_building(self):
        if not self.enhanced_building:
            return None
        bases = [
            b for b in list(self.tribe.buildings.values()) if b.enhancement == self.name
        ]
...
class Tribe(object):
    def __init__(self, tribeinfo, json_directory):
        ...
        with open(
            p.normpath(json_directory + "/" + self.name + "_wares.json"), "r"
        ) as wares_file:
        ...
        with open(
            p.normpath(json_directory + "/" + self.name + "_workers.json"), "r"
        ) as workers_file:
        ...
        with open(
            p.normpath(json_directory + "/" + self.name + "_buildings.json"), "r"
        ) as buildings_file:
