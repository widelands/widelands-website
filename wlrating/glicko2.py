...
class Glicko_rating ():
    def __init__(self):
        self.starting_score = Decimal(
            settings.STARTING_SCORE).quantize(Decimal('1.00'))
        self.standard_deviation = Decimal(
            settings.STANDARD_DEVIATION).quantize(Decimal('1.00'))
        self.volatility = Decimal(
            settings.VOLATILITY).quantize(Decimal('1.00000'))
    ...
    def step3_nu(self, games, mu):
        total_sum = 0
        for game in games:
            E = self.step3_E(mu, game['competitor']
                             ['score'], game['competitor']['deviation'])
    ...
    def step5_function_f(self, x, Delta, phi, nu, sigma):
        numerator1 = Delta**Decimal(2) - \
            phi**Decimal(2) - nu - Decimal(math.exp(x))
    ...
    def step6_phi_star(self, phi, sigma_prime):
        phi_star = (phi**Decimal(2) + Decimal(sigma_prime)
                    ** Decimal(2))**Decimal(0.5)
        return phi_star

    def step7_phi_prime(self, phi_star, nu):
        phi_prime = Decimal(1)/((Decimal(1)/phi_star**2) +
                                (Decimal(1)/nu))**Decimal(0.5)
        return phi_prime
    ...
    def step7_mu_prime(self, games, mu, phi_prime):
        ...
        for game in games:
            E = self.step3_E(mu, game['competitor']
                             ['score'], game['competitor']['deviation'])
            total_sum += self.step3_g(game['competitor']
                                      ['deviation']) * (game['is_winner'] - E)
